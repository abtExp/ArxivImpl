...
from tensorflow.keras.layers import Layer, UpSampling2D, Add,\
...
class OCTCONV_LAYER(Layer):
    ...
    def call(self, x):
        ...
        low_inp = K.pool2d(
            low_inp, (2, 2), strides=self.strides, pool_mode='avg')

        out_high_high = K.conv2d(high_inp, self.h_2_h,
								strides=self.strides, padding=self.padding)
        out_low_high = UpSampling2D((2, 2))(
            K.conv2d(low_inp, self.l_2_h, strides=self.strides, padding=self.padding))
        out_low_low = K.conv2d(low_inp, self.l_2_l,
							strides=self.strides, padding=self.padding)
        out_high_low = K.pool2d(
            high_inp, (2, 2), strides=self.strides, pool_mode='avg')
        out_high_low = K.conv2d(out_high_low, self.h_2_l,
                                strides=self.strides, padding=self.padding)
