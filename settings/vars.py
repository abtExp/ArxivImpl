...
from utils import yolo_utils, enet_utils, utils, scfegan_utils
import numpy as np
...
USE_LIAO = False
DEBUG = False
LOADED_ORIENTATION_ENCODER = False
USE_PRETRAINED_ORIENTATION_ENCODER = True
...
def get_callbacks(model='enet'):
	if model == 'enet':
			formatter = enet_utils.enet_outputs
			plotter = enet_utils.plot
	elif model == 'yolo':
			formatter = yolo_utils.yolo_outputs
			plotter = yolo_utils.plot

	eval_cb = utils.EvaluationCallback(model, INP_SHAPE, formatter, plotter, FLIKR_ONLY_LOGO_IDS)
	path = './checkpoints/checkpoints_{}/'.format(model)
	checkpoint = ModelCheckpoint(path+'weights.{epoch:02d}-{val_loss:.2f}.hdf5', monitor='val_loss', verbose=0, save_best_only=True)
	earlystop = EarlyStopping(monitor='val_loss', min_delta=0, patience=3, verbose=0)
	tensorboard = TensorBoard(log_dir='./logs/logs_{}/'.format(model), histogram_freq=0, batch_size=32, write_graph=True, write_grads=True, write_images=True)
	reducelr = ReduceLROnPlateau(monitor='val_loss', factor=0.01, patience=1, verbose=0, mode='auto', min_delta=0.0001, cooldown=0, min_lr=0)

	return [checkpoint, earlystop, tensorboard, reducelr, eval_cb]
...
YOLO_DATA_LOADER = utils.load_valid_data
...
CCNGAN_DATA_LOADER = utils.ccngan_data_loader
...
# CGAN
GAN_DATA_LOADER = utils.gen_loader
GEN_LOADER_TEST_BATCH_SIZE = 10

SCFEGAN_BATCH_SIZE = 1
...
SCFEGAN_DATA_LOADER = scfegan_utils.gen_inp
LRN_LAYER = scfegan_utils.LRNLayer
GATED_DE_CONV = scfegan_utils.GatedDeConv

ORIENTATION_ENCODER = utils.orientation_encoder
GET_ORIENTATION_ENCODER = utils.get_orientation_encoder